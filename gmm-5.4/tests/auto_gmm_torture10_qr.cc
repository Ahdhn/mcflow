...
int main(void) {
  ...
  for (int iter = 0; iter < 100000; ++iter) {
    ...
    try {
      ...
      gmm::col_matrix<std::vector<std::complex<float> > >  param0(15, 14);
      gmm::fill_random(param0, 0.2);
      gmm::row_matrix<gmm::wsvector<std::complex<float> > >  param1(62, 14);
      gmm::fill_random(param1);
    

      bool ret = test_procedure(gmm::sub_matrix(param0, gmm::sub_interval(5, 10), gmm::sub_interval(0, 14)), gmm::sub_matrix(param1, gmm::sub_slice(12, 14, 1), gmm::sub_interval(0, 14)));
      if (ret) return 0;
